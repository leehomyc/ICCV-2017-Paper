...
%\subsection{Facial Capture and Mouth Modeling}
%Existing facial capture system either relies on image or video. 
%Single view methods such as ~\cite{blanz1999,k-s2011} use morphable model fail to capture fine-scale texture details. Other methods which output fine 3D details require more prior knowledge and constraints. For example,~\cite{Bee10} uses seven cameras to acquire high-quality geometry in a single shot. \\
...
%In recent years deep neural networks rise as a powerful tool that could accomplish tasks previously thought impossible, from image classification ~\cite{imagenet_cvpr09} to semantically transferring style ~\cite{style} and high-quality textures synthesis~\cite{saito2016}. 
%Most related to our work is recent advances of image synthesis using deep auto-encoders ~\cite{autoencoders}, in particular the generative adversarial networks~\cite{gan} which is capable of generating sharp and realistic images. 
%More specifically, we use conditional GAN~\cite{pix2pix,cgan,gauthier2014} to enable end-to-end synthesize of high-resolution face texture conditioned on dual identity and expression facial inputs.
